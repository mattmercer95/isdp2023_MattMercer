...
window.onload = async function () {
    ...
    document.querySelector("#acceptOrder").addEventListener('click', acceptOrder);
...
async function acceptOrder(){
    let completeConfirm = confirm("Accept order?");
    if(!completeConfirm){
        return;
    }
    let url = `../TransactionService/complete`;
    ...
    if(s1){
        alert(`Order #${currentOrder.transactionID} successfully completed`);
